...
namespace constexpression {
        namespace detail {
            ...
            public:
                ...
                constexpr explicit compare_key(const Key& key_) noexcept : key(key_) {
                }
            ...
            public:
                ...
                constexpr pair_element(const key_type& key, const mapped_type& value) noexcept : pair(key, value) {
                }

                constexpr bool operator<(const pair_element& rhs) const noexcept {
                ...
                constexpr void swap(pair_element& rhs) noexcept {
            ...
            class element_hash : public pair_element<Key, Value> {
                using base_type = pair_element<Key, Value>;
        ...
        namespace detail {
            ...
            public:
                template <typename T>
                constexpr map(const T (&data)[N]) noexcept : map(data, std::make_index_sequence<N>()) {
                }
        ...
        static constexpr auto map(const std::pair<const Key, const Value> (&items)[N]) noexcept {
            return detail::map<detail::pair_element<Key, Value>, N>(items);
        }
        ...
        static constexpr auto hash_map(const std::pair<const Key, const Value> (&items)[N]) noexcept {
            return detail::map<detail::element_hash<Key, Value, Hasher>, N>(items);
        }

    }

namespace constexpression::eternal {
        namespace impl {
...
namespace std {
    ...
    template <>
    struct hash<::constexpression::eternal::string> {
        constexpr std::size_t operator()(const ::constexpression::eternal::string& str) const {
            return ::constexpression::eternal::impl::str_hash(str.data());
        }
    };
