...
public:
    explicit Parser(std::string_view data) : lex(data) {}

private:
    inline bool expect(OpCode code) {
        return lex.is<Tok::STRING>(opCodeToString(code));
    }
    ...
    InstructionList parseInstructions() {
        ...
        while (true) {
            ...
            if (expect(OpCode::AND)) {
                ...
                list.emplace_back(Instruction::CALL(module, method));
                continue;
                ...
                list.emplace_back(Instruction::LDC(intgr)); //Todo unsafe cast
                continue;
                ...
                list.emplace_back(Instruction::PUTFIELD(s));
                continue;
            } else if (expect(OpCode::GETFIELD)) {
                lex.skipSpaces();
                auto s = get<Tok::STRING>("expect field name");
                list.emplace_back(Instruction::GETFIELD(s));
                continue;
            } else if (expect(OpCode::NEW)) {
                lex.skipSpaces();
                auto s = get<Tok::STRING>("expect class name");
                list.emplace_back(Instruction::NEW(s));
                continue;
